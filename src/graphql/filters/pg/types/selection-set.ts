...
export class SelectionInput {
  private readonly selectionSetArray: string[] = [];
  private readonly selectionSetObject: any = {};
  private readonly selectionSetRelations: any = {};
  ...
  /**
   * Returns the complete hierarchical structure of selected fields.
   * Example for query: { user { name, email, sessions { id, ip } } }
   * Returns: { user: { name: {}, email: {}, sessions: { id: {}, ip: {} } } }
   * 
   * @returns {any} The hierarchical structure of selected fields
   */
  getSelectionSetStructure = (): any => {
    return this.selectionSetObject;
  };
  ...
  private readonly getSelectionSetObject = (selections: readonly any[], variables: any): any => {
    const fields: any = {};
      ...
      const shouldInclude =
        (!includeDirective ||
          includeDirective.arguments?.some(
            (arg) => arg?.name?.value === 'if' && variables?.[arg?.value?.name?.value]
          )) &&
  ...
  private readonly getSelectionSetRelations = (selections: readonly any[], variables: any): any => {
    const fields: any = {};
  ...
  private readonly getSelectionSetArray = (selections: readonly any[], variables: any): string[] => {
    const fields: string[] = [];
  ...
  private readonly getSeletionSetRoot = (selections: readonly any[], root: string): any => {
